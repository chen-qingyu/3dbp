...
pub fn parse_input(input_file: &str) -> Input {
    // 读取文件
    let content = fs::read_to_string(input_file).unwrap_or_else(|_| {
        error!("Failed to open input file \"{}\".", input_file);
        process::exit(1);
    });
    let input_json: serde_json::Value = serde_json::from_str(&content).unwrap();
    ...
    // 关联box与box_type
    let box_type_map: std::collections::HashMap<String, Rc<_>> = input
        .box_types
        .iter()
        .map(|bt| (bt.id.clone(), Rc::new(bt.clone())))
        .collect();
    for box_item in &mut input.boxes {
        if let Some(box_type) = box_type_map.get(&box_item.type_id) {
            box_item.box_type = Some(Rc::clone(box_type));
            box_item.lx = box_type.lx;
            box_item.ly = box_type.ly;
            box_item.lz = box_type.lz;
        }
    }
