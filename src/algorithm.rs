...
impl Algorithm {
    ...
    /// 尝试装载箱子
    fn try_pack(&self, box_item: &mut Box) -> bool {
        ...
        // 克隆orients避免借用冲突
        let orients = box_item.box_type.as_ref().unwrap().orients.clone();
        // 尝试每个允许的方向
        for orient in orients {
            box_item.set_orient(orient);
            ...
            for &(x, y, z) in &candidates {
                ...
                // 检查所有约束
                if constraint.check_constraints(box_item) {
                    return true; // 找到有效位置
                }
            }
        }
    ...
    pub fn run(mut self) -> Output {
        ...
        while !self.unpacked_boxes.is_empty() {
            ...
            // 尝试装载箱子
            let boxes_to_try: Vec<Box> = self.unpacked_boxes.to_vec();
            for mut box_item in boxes_to_try {
                if self.try_pack(&mut box_item) {
                    self.packed_boxes.push(box_item);
                }
            }
        ...
        // 重置未装载箱子的位置
        for box_item in &mut self.unpacked_boxes {
            box_item.x = None;
            box_item.y = None;
            box_item.z = None;
        }
